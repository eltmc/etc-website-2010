[%
    SET title = "Edinburgh and Lothians Twins and Multiples Club :: Join";
    SET root = "../";
    INCLUDE 'header.inc';
    INCLUDE 'sidebar.inc';
%]

    <div id="content">
    <h1 class="header">Why Join Edinburgh Twins Club?</h1>
        <p>
If you join the Edinburgh Twins Club (for the small cost of &pound;12 pa), you will get the following benefits:</p>

<ul>
  <li>Discounts from a number of companies (2 visits to Clarks shoe shop will pay for your membership fee !)</li>
  <li>Support from people who are in the same position as yourself</li>
  <li>Regular Newsletters</li>
  <li>Organised Social Events</li>
  <li>Weekly Bumps, Babies and Toddler sessions</li>
  <li>"Swap Shop" or "Wanted" - details of equipment, toys and clothes for sale or wanted by members</li>
  <li>Library of books and leaflets</li>
</ul>
<p>
Download the application form here in <a href="[% root %]documents/application-form.pdf">PDF</a> / <a href="[% root %]documents/application-form.doc">MS Word</a> format.
</p>

<div class="elastic-photo-bar">
<img src="[% root %]graphics/photos/join.jpg" alt="twins on a slide"/>
</div>

</div>
[% INCLUDE 'footer.inc'; %]
